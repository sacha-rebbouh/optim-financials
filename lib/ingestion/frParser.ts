...
const HEADER_ALIASES = {
  date: ["date", "date opération", "date operation", "date comptable"],
  merchant: ["libellé", "libelle", "libellé opération", "description", "motif"],
  amount: ["montant", "montant opération", "débit", "credit", "crédit"],
  currency: ["devise", "currency"],
};

export function parseFrenchRows(rows: string[][]): FrParseResult {
  if (rows.length === 0) {
  ...
  if (headerIndex === -1) {
    return {
      transactions: [],
      warnings: ["En-têtes non détectés pour format FR."],
    };
  }
...
function findHeaderRow(rows: string[][]) {
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (row.includes("date") && (row.includes("libell") || row.includes("motif"))) {
      return i;
    }
  }
  return -1;
